...
// Define styles that match the professional invoice format
const styles = StyleSheet.create({
  ...
  col1: { width: '8%', textAlign: 'center', paddingTop: 2 },
  col2: { width: '32%', paddingTop: 2 },
  col3: { width: '10%', textAlign: 'center', paddingTop: 2 },
  col4: { width: '12%', textAlign: 'center', paddingTop: 2 },
  col5: { width: '13%', textAlign: 'right', paddingTop: 2 },
  col6: { width: '10%', textAlign: 'center', paddingTop: 2 },
  col7: { width: '15%', textAlign: 'right', paddingTop: 2 },
  totalsContainer: {
    flexDirection: 'row',
  ...
  taxCol1: { width: '12%', textAlign: 'center', paddingTop: 1 },
  taxCol2: { width: '16%', textAlign: 'right', paddingTop: 1 },
  taxCol3: { width: '12%', textAlign: 'center', paddingTop: 1 },
  taxCol4: { width: '16%', textAlign: 'right', paddingTop: 1 },
  taxCol5: { width: '12%', textAlign: 'center', paddingTop: 1 },
  taxCol6: { width: '16%', textAlign: 'right', paddingTop: 1 },
  taxCol7: { width: '16%', textAlign: 'right', paddingTop: 1 },
  footer: {
    flexDirection: 'row',
    marginTop: 10,
    borderTop: '1px solid #999',
...
const InvoicePDF: React.FC<InvoicePDFProps> = ({
  ...
  items.forEach(item => {
    const hsn = item.hsn || '-'
    const gstPercent = item.gstPercent || 0 // Use 0 if no GST specified
    
    // Calculate taxable amount and tax amounts based on actual GST percentage
    let taxableAmount, totalTaxAmount, cgstAmount, sgstAmount
    
    if (gstPercent > 0) {
      // If GST is specified, calculate tax-inclusive amounts
      taxableAmount = item.lineTotal / (1 + gstPercent / 100)
      totalTaxAmount = item.lineTotal - taxableAmount
      cgstAmount = totalTaxAmount / 2
      sgstAmount = totalTaxAmount / 2
    } else {
      // If no GST, the line total is the taxable amount
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.container}>
          ...
          <View style={styles.header}>
            ...
            <View style={styles.invoiceBlock}>
              ...
              {transactionId && (
                <Text style={styles.invoiceMeta}>Transaction ID: {transactionId}</Text>
              )}
        ...
        {generatedBy ? (
          <Text fixed style={styles.generatedByFooter}>Invoice Generated By: {generatedBy}</Text>
        ) : null}
      </Page>
    </Document>
  )
