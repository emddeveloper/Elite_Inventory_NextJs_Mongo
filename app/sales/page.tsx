...
export default function SalesPage() {
	...
	const addItem = (p: Product) => {
		...
		setItems(prev => [...prev, { productId: p._id, name: p.name, sku: p.sku, unitPrice: p.price, quantity: 1, lineTotal: p.price }])
	}
	...
	const submit = async () => {
		...
		try {
			...
			if (res.ok) {
				...
				// Download invoice
				const pdf = await fetch(`/api/transactions/${data.transaction._id}/invoice`)
				const blob = await pdf.blob()
				const url = URL.createObjectURL(blob)
				const a = document.createElement('a')
				a.href = url
				a.download = `${data.transaction.invoiceNumber}.pdf`
				a.click()
