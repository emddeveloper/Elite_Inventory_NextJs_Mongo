...
export async function GET() {
  try {
    ...
    const dailySalesAgg = await Transaction.aggregate([
      { $match: { createdAt: { $gte: todayStart, $lte: todayEnd } } },
      { $group: { _id: null, totalSales: { $sum: '$total' } } }
    ])
    const dailySales = (dailySalesAgg[0] && dailySalesAgg[0].totalSales) || 0
    ...
    // Format currency for stat display (exactly two decimals)
    const fmtCurrency = (n: number) => new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(n)
    ...
    const stats = [
      ...
      { name: 'Daily Sales', stat: fmtCurrency(dailySales), change: '0%', changeType: 'increase' },
      { name: 'Monthly Sales', stat: fmtCurrency(monthlySales), change: '0%', changeType: 'increase' }
    ]
